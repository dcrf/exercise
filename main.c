...
#define APP_SERVER_RESERVED_PORT_RANGE      (1024)  // First 1024 ports are reserved for Admin services
...
int main(int argc, char **argv)
{
    ...
    else if (APP_SERVER_MODE == app_mode)
    {
        ...
        result = run_application_in_server_mode(p_port, &app_run);
    }
...
static app_mode_t app_identify_running_mode(int argc, char *argv[])
{
        ...
        (strcmp("client", argv[APP_OPERATION_MODE_INDEX]) == 0))
    {
        ...
        mode = (port <= APP_SERVER_RESERVED_PORT_RANGE) ? APP_UNKNOWN_MODE : APP_CLIENT_MODE;
    }
    else if (strcmp("server", argv[APP_OPERATION_MODE_INDEX]) == 0)
    {
        ...
        mode = (port <= APP_SERVER_RESERVED_PORT_RANGE) ? APP_UNKNOWN_MODE : APP_SERVER_MODE;
    }
    else
    {
...
static long int convert_port_string_to_numeric(const char *p_port_value)
{
    ...
    if (port <= APP_SERVER_RESERVED_PORT_RANGE)
    {
        printf("\r\nThe port value [%ld] is invalid. It should be greater than %d", port, APP_SERVER_RESERVED_PORT_RANGE);
    }
